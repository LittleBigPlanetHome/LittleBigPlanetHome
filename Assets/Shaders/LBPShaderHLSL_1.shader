Shader "LBP" {
    ...
    SubShader {
        ...
        void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
            ...
            float4 secondColor = tex2D(_SecondTex, IN.uv);
            float4 thirdColor = tex2D(_ThirdTex, IN.uv);
            float4 fourthColor = tex2D(_FourthTex, IN.uv);
            float4 fifthColor = tex2D(_FifthTex, IN.uv);
            float4 sixthColor = tex2D(_SixthTex, IN.uv);
            float4 seventhColor = tex2D(_SeventhTex, IN.uv);
            float4 eighthColor = tex2D(_EighthTex, IN.uv);
        ...
        float4 colorBuffer = tex2D(_ColorBuffer, IN.uv);
        float4 shadowColor = tex2D(_ShadowTex, IN.uv);
